"""
metrics.py — Cálculo de métricas para la evaluación del modelo FINE-TUNED
===========================================================================
Lee el CSV detallado generado por processor.py (respuestas del modelo
fine-tuneado + tiempos) y calcula métricas de calidad de texto:
EM, TokenOverlap, LengthRatio, BLEU-1/2/4, ROUGE-1/2/L, METEOR,
BERTScore (P/R/F1) y similitud semántica (SBERT).

Genera 2 CSV de salida:
    1) metrics_output          -> métricas por cada iteración/pregunta
    2) metrics_output_summary  -> promedios por pregunta y promedio global
"""
...
# Leer la configuración generada por processor.py (config.py se reescribe en cada corrida)
from config import (
    model_name,
    output_filename,          # ENTRADA para este script (CSV de processor.py)
...
# 1. Configuración inicial y carga de modelos de métricas
def cargar_recursos():
    ...
    recursos = {
        ...
        'smoother': SmoothingFunction().method2,  # Método específico para BLEU
...
# 3. Extracción de datos del CSV de resultados generado por processor.py
        ...
        with open(filepath_csv, "r", encoding="utf-8", newline='') as f:
            ...
            for row_num, row in enumerate(reader):
                try:
                    ...
                    # Soporta tanto "Pregunta_Texto" como "Pregunta" (nombre usado por processor.py)
...
# 4. Cálculo de métricas entre respuesta esperada (referencia) y respuesta del modelo fine-tuned
def calcular_metricas_finetuning(ref, pred, recursos):
    ...
    if pred_norm and ref_norm:
        try:
            ...
            metricas.update({'ROUGE-1': 0.0, 'ROUGE-2': 0.0, 'ROUGE-L': 0.0})
    else:
        metricas.update({'ROUGE-1': 0.0, 'ROUGE-2': 0.0, 'ROUGE-L': 0.0})

    if pred_norm and ref_norm:
        try:
            metricas['METEOR'] = recursos['meteor'].compute(predictions=[pred_norm], references=[ref_norm])['meteor']
        except Exception:
            metricas['METEOR'] = 0.0
    else:
        metricas['METEOR'] = 0.0
    ...
    if pred_norm and ref_norm:
        try:
            ...
            metricas.update({'BERT-P': 0.0, 'BERT-R': 0.0, 'BERT-F1': 0.0})
    else:
        metricas.update({'BERT-P': 0.0, 'BERT-R': 0.0, 'BERT-F1': 0.0})
    ...
    if ref and pred:
        try:
            ...
            metricas['SemanticSim'] = 0.0
    else:
        metricas['SemanticSim'] = 0.0
    return metricas


# 5. Función principal para procesar y generar los archivos CSV de métricas
def procesar_y_generar_archivos_metricas(filepath_input_csv):
    ...
    with open(metrics_output, "w", encoding="utf-8", newline='') as f_detallado_csv:
        ...
        for preg_id, pregunta_texto, respuesta_esperada, iteraciones in datos_de_preguntas:
            ...
            for iter_num, respuesta_generada, tiempo_generacion in iteraciones:
                metricas_iteracion = calcular_metricas_finetuning(respuesta_esperada, respuesta_generada, recursos)
        ...
        for preg_id, data_pregunta in sorted(metricas_por_pregunta.items()):
            ...
            for nombre_metrica in ORDERED_METRIC_NAMES:
                ...
                if valores_metrica:
                    ...
                    fila_resumen_pregunta[nombre_metrica] = 0.0
        ...
        if todas_las_metricas_globales:
            ...
            for nombre_metrica in ORDERED_METRIC_NAMES:
                ...
                if valores_metrica_global:
                    ...
                    fila_resumen_global[nombre_metrica] = 0.0
    ...
    print(f"\n{'=' * 60}")
    print("Evaluación de Métricas Completada (Fine-Tuning)")
    ...
    if todos_los_tiempos_globales:
        print(f"Respuestas evaluadas: {len(todos_los_tiempos_globales)}")
...
if __name__ == "__main__":
    print(f"Iniciando script de métricas (Fine-Tuning) para el modelo: {model_name}")
